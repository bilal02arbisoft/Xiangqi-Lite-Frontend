...
const BoardPage = () => {
  ...
  const [selectedSquareInfo, setSelectedSquareInfo, latestSelectedSquareInfo] =
    useState();
  ...
  async function fetchUserDetails() {
    try {
      ...
      const response = await axios.get(
        `${config.BACKEND_HTTP_URL}/api/profile/`,
        {
          headers: {
    ...
    try {
      ...
      if (!gameIdFromParams) {
        const response = await axios.post(
          `${config.BACKEND_HTTP_URL}/game/create/`,
          null,
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  function handleGameEnd(result) {
    ...
    const winner = result;
    const loser =
      latestRed.current.username === winner
        ? latestBlack.current.username
        : latestRed.current.username;
    setGameResult({
      winner: winner,
      loser: loser,
    });
  ...
  function handleSelectSquare(row, column) {
    ...
    if (index !== -1) {
      ...
      setSelectedSquareInfo((prevState) => ({
        ...prevState,
        ...newSqr[index],
      }));
  ...
  function handleMovePiece(piece, color, row, column) {
    ...
    if (move && wsManagerRef.current && wsManagerRef.current.isConnected) {
      ...
      const thinkingTime = Math.floor(
        (Date.now() - latestTurnStartTime.current) / 1000,
      );
