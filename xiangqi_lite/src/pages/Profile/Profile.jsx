...
const ProfileEditPage = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      axios
        .get(`${config.BACKEND_HTTP_URL}/api/profile/`, {
          headers: {
        ...
        .then((response) => {
          ...
          setProfilePictureUrl(
            profileData.profile.profile_picture
              ? `${config.BACKEND_HTTP_URL}${profileData.profile.profile_picture}`
              : "",
          );
          setOriginalData({
      ...
      axios
        .get(`${config.BACKEND_HTTP_URL}/friendship/friends/`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          withCredentials: true,
  ...
  const handleRequestOtp = () => {
    ...
    axios
      .post(
        `${config.BACKEND_HTTP_URL}/api/requestotp/`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleSaveChanges = (e) => {
    ...
    if (rating !== originalData.rating)
      formData.append("profile.rating", rating);
    if (country !== originalData.country)
      formData.append("profile.country", country);
    if (skillLevel !== originalData.skill_level)
      formData.append("profile.skill_level", skillLevel);
    ...
    const token = localStorage.getItem("access_token");
    axios
      .put(`${config.BACKEND_HTTP_URL}/api/profile/`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      .then((response) => {
        ...
        setProfilePictureUrl(
          updatedProfile.profile.profile_picture
            ? `${config.BACKEND_HTTP_URL}${updatedProfile.profile.profile_picture}`
            : "",
        );
      })
      .catch((error) => {
  ...
  const handleDeleteAccount = () => {
    if (window.confirm("Are you sure you want to delete your account?")) {
      const token = localStorage.getItem("access_token");
      axios
        .delete(`${config.BACKEND_HTTP_URL}/api/delete/`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          withCredentials: true,
                ...
                <div key={index} className="flex items-center space-x-4">
                  <div className="w-12 h-12 rounded-full bg-gray-300 flex justify-center items-center overflow-hidden">
                    {friend.profile.profile_picture ? (
                      <img
                        src={`${config.BACKEND_HTTP_URL}${friend.profile.profile_picture}`}
                        alt={friend.username}
                        className="w-full h-full object-cover"
                      />
                    ) : (
