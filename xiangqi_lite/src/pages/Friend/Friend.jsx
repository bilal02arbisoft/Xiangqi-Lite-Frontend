...
const FriendsPage = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://127.0.0.1:8000/friendship/friends/", {
        headers: {
    ...
    axios
      .get("http://127.0.0.1:8000/friendship/requests/", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        withCredentials: true,
  ...
  const handleAccept = (request) => {
    ...
    axios
      .post(
        "http://127.0.0.1:8000/friendship/accept-request/",
        { from_user: username },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleReject = (username) => {
    ...
    axios
      .post(
        "http://127.0.0.1:8000/friendship/reject-request/",
        {
          from_user: username,
        },
        {
  ...
  const handleSearch = () => {
    ...
    axios
      .get(
        `http://127.0.0.1:8000/friendship/users/search/?query=${searchQuery}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleSendRequest = (username) => {
    ...
    axios
      .post(
        "http://127.0.0.1:8000/friendship/send-request/",
        {
          username: username,
        },
        {
      ...
      .then((response) => {
        ...
        setSearchResults((prevResults) =>
          prevResults.filter((result) => result.username !== username),
        );
      })
      .catch((error) => {
                  ...
                  <div key={index} className="flex items-center space-x-4">
                    <div className="w-12 h-12 rounded-full bg-gray-300 flex justify-center items-center overflow-hidden">
                      {friend.profile.profile_picture ? (
                        <img
                          src={`http://127.0.0.1:8000${friend.profile.profile_picture}`}
                          alt={friend.username}
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="w-12 h-12 rounded-full bg-gray-300 flex justify-center items-center overflow-hidden">
                        {request.profile.profile_picture ? (
                          <img
                            src={`http://127.0.0.1:8000${request.profile.profile_picture}`}
